import{LitElement,html,css}from"https://unpkg.com/lit@2.0.0/index.js?module";import{styleMap}from"https://unpkg.com/lit@2.0.0/directives/style-map.js?module";import{subscribeEntities}from"https://unpkg.com/home-assistant-js-websocket@9.6.0/dist/index.js";const CARD_VERSION="0.1.9";console.info("%c CHRONO-%cPOPUP %c v0.1.9 ","background-color: #101010; color: #FFFFFF; font-weight: bold; padding: 2px 0 2px 4px; border-radius: 3px 0 0 3px;","background-color: #101010; color: #4676d3; font-weight: bold; padding: 2px 0 2px 0;","background-color: #1E1E1E; color: #FFFFFF; font-weight: bold; padding: 2px 4px; border-radius: 0 3px 3px 0;");const EVENT_KEY="chrono-popup",KNOWN_DATA_KEYS=["title","view","styles","dismissable"],DEFAULT_STYLES={width:"auto",minWidth:"580px",maxWidth:"90vw",height:"auto",minHeight:"533px",maxHeight:"90vh",background:"var(--card-background-color, #1c1c1c)",borderRadius:"12px"};class ChronoPopupHost extends LitElement{static properties={_open:{state:!0},_loading:{state:!0},_error:{state:!0},_opts:{state:!0},_view:{state:!0}};constructor(){super(),this._open=!1,this._loading=!1,this._error=null,this._opts={},this._view=null,this._hassUnsub=null,this._attachedView=null,this._onKeydown=this._onKeydown.bind(this)}connectedCallback(){super.connectedCallback(),document.addEventListener("keydown",this._onKeydown)}disconnectedCallback(){document.removeEventListener("keydown",this._onKeydown),this._unsubscribeFromUpdates(),super.disconnectedCallback()}_onKeydown(e){"Escape"===e.key&&this._open&&this.close()}_getHass(){const e=document.querySelector("home-assistant");return e?e.hass:void 0}async _subscribeToUpdates(e){if(this._unsubscribeFromUpdates(),e?.connection)try{this._hassUnsub=await subscribeEntities(e.connection,()=>{const e=this.renderRoot?.querySelector("hui-view");e&&(e.hass=this._getHass())})}catch(e){console.warn("chrono-popup: could not subscribe to entity updates - popup content will not update live",e)}}_unsubscribeFromUpdates(){if("function"==typeof this._hassUnsub)try{this._hassUnsub()}catch(e){}this._hassUnsub=null}async open(e={}){for(const t of Object.keys(e))KNOWN_DATA_KEYS.includes(t)||console.warn(`chrono-popup: unrecognized key "${t}" in event_data (view: "${e.view||"?"}"). Recognized keys: ${KNOWN_DATA_KEYS.join(", ")}. CSS goes under "styles:".`);this._opts={title:e.title??"",dismissable:!1!==e.dismissable,styles:e.styles&&"object"==typeof e.styles?e.styles:{}},this._error=null,this._view=null,this._attachedView=null,this._open=!0,this._loading=!0;const t=this._getHass();t&&this._subscribeToUpdates(t);try{this._view=await this._resolveView(e.view)}catch(e){this._error=e.message||String(e)}finally{this._loading=!1}}close(){this._open=!1,this._unsubscribeFromUpdates()}async _resolveView(e){if(!e||"string"!=typeof e)throw new Error('chrono-popup: "view" is required, e.g. "/my-dashboard/my-view"');const t=e.split("/").filter(Boolean);if(t.length<2)throw new Error(`chrono-popup: "view" must include both a dashboard and a view, e.g. "/my-dashboard/my-view" (got "${e}")`);const[s,i]=t,o=this._getHass();if(!o)throw new Error("chrono-popup: could not access hass - is Home Assistant fully loaded?");let r;try{r=await o.callWS({type:"lovelace/config",url_path:s})}catch(e){throw new Error(`chrono-popup: could not load dashboard "${s}" (${e.message||e})`)}const n=Array.isArray(r.views)?r.views:[],a=n.find(e=>e.path===i);if(!a)throw new Error(`chrono-popup: view "${i}" not found in dashboard "${s}"`);if(!this._isViewVisibleToUser(a,o))throw new Error(`chrono-popup: view "${i}" is not visible to the current user`);return{lovelace:{config:r,urlPath:s,editMode:!1},index:n.indexOf(a),viewConfig:a}}_isViewVisibleToUser(e,t){const s=t?.user?.id||null;if(!s||!e)return!0;const i=(Array.isArray(e.visible)&&e.visible.length?e.visible:null)||(Array.isArray(e.visibility)&&e.visibility.length?e.visibility:null)||(Array.isArray(e.users)&&e.users.length?e.users:null);if(!i)return!0;for(const e of i){if("string"==typeof e&&e===s)return!0;if(e&&"object"==typeof e){if("string"==typeof e.user&&e.user===s)return!0;if(Array.isArray(e.user)&&e.user.includes(s))return!0;if(Array.isArray(e.users)&&e.users.includes(s))return!0}}return!1}static styles=css`
  ...
  `;render(){if(!this._open)return html``;const{title:e,dismissable:t,styles:s}=this._opts;return html`
      ...
      >
        <div
          class="frame"
          style=${styleMap({...DEFAULT_STYLES,...s})}
        >
          <div class="header">
          ...
          <div class="body">
            ...
            ${this._loading||this._error||!this._view?"":html`<div class="view-container"></div>`}
          </div>
        </div>
      </div>
    `}updated(){if(!this._open||this._loading||this._error||!this._view)return;if(this._attachedView===this._view)return;const e=this.renderRoot?.querySelector(".view-container");if(!e)return;const t=document.createElement("hui-view");t.setAttribute("style","margin:0;padding:0;display:contents;"),t.hass=this._getHass(),t.narrow=!1,t.lovelace=this._view.lovelace,t.index=this._view.index,t.isStrategyView=!1,t.viewConfig=this._view.viewConfig,e.innerHTML="",e.appendChild(t),this._attachedView=this._view}}if(customElements.get("chrono-popup-host")||customElements.define("chrono-popup-host",ChronoPopupHost),!window.__chronoPopupHostInstalled){window.__chronoPopupHostInstalled=!0;const e=document.createElement("chrono-popup-host");document.body.appendChild(e),document.addEventListener("ll-custom",t=>{const s=t.detail&&t.detail[EVENT_KEY];s&&e.open(s.data||{})})}